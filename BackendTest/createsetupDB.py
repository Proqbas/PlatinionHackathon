...
with app.app_context():
    ...
    t1 = Task(name="CreateGit", desc="Collab is usefull, less work for me", status="PLANNED")
    db.session.add(t1)
    t2 = Task(name="CreateUI", desc="Frontend with buttons", status="DONE")
    db.session.add(t2)
    t3 = Task(name="Budgeting", desc="Something important with money", status="PLANNED")
    db.session.add(t3)
    t4 = Task(name="CreateDB", desc="Create a Database", status="PLANNED")
    db.session.add(t4)
    t5 = Task(name="Ideating", desc="Think about something new", status="PLANNED")
    db.session.add(t5)
